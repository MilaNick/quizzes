...
import { TState, TQuizzes } from './interface.js';
...
const renderAnswer = (dataQuiz) => {
    return dataQuiz.answers.map(answer => {
        console.log('answer', answer)
        // btn-${item.correctly ? 'correct' : 'incorrect'
        return (`
                <button class="btn btn-grad" value=${answer.id}>${answer.value}</button>
            `)
    })
}
...
const renderPage = () => {
    ...
    switch (state.pageType) {
        ...
        case 'quiz':
            ...
            ${keys.map(key => {
                return(
                    `<div class="question-wrap"><div class="question" id=${i}>${data.quizzes[key].tasks[i].question}</div></div>
